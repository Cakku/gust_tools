...
int main_utf8(int argc, char** argv)
{
    ...
    if (strstr(argv[argc - 1], ".json") != NULL) {
        printf("Converting to EBM.. '%d' files\n", argc - 1);
        ...
        printf("Converting to JSON.. '%d' files\n", argc -1);
        for (int i = 1; i < argc; i++)
        {
            convert_ebm(argv[i]);
        }
        ...
        fprintf(stderr, "ERROR: You must specify a .ebm or .json file(s)");
    }
...
int convert_json(char* input_path)
{
    ...
    json = json_parse_file_with_comments(input_path);
    if (json == NULL) {
    ...
    snprintf(path, sizeof(path), "%s%c%s", _dirname(input_path), PATH_SEP,
...
int convert_ebm(char* input_path)
{
    ...
    uint32_t buf_size = read_file(input_path, &buf);
...
detected:
    ...
    json_object_set_string(json_object(json), "name", _basename(input_path));
    ...
    snprintf(path, sizeof(path), "%s%c%s", _dirname(input_path), PATH_SEP,
        change_extension(_basename(input_path), ".json"));
    printf("Creating '%s'\n", path);
